...
const soloNombres = (listaDePersonas) => {
...
const promedioEdades = function(listaDePersonas) {
 ...
 */
// Usando arrow function
const cumplirAños = (listaDePersonas) => {
 ...
 */
// Usando expresión de función
const laPersonaMayor = function(listaDePersonas) {
    if (listaDePersonas.length === 0) {
        return null;
    }
 ...
 */
// Usando arrow function
const agregarHeladoFavorito = (listaDePersonas, listaDeHelados) => {
    return listaDePersonas.map((persona, indice) => {
        const helado = indice < listaDeHelados.length ? listaDeHelados[indice] : "vainilla";
        return {
            ...persona,
